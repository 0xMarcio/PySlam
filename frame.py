...
def extractFeatures(img):
    orb = cv2.ORB_create()
    # detection
    # pts = cv2.goodFeaturesToTrack(np.mean(img, axis=2).astype(np.uint8), 1000, qualityLevel=0.5, minDistance=7)
    pts = cv2.goodFeaturesToTrack(np.mean(img, axis=2).astype(np.uint8), 500, 0.1, 7)
        ...
        for i1 in range(len(self.kpus)):
            ...
            if self.pts[i1] is not None:
                ...
                for f, idx in zip(self.pts[i1].frames[-9:][::-1], self.pts[i1].idxs[-9:][::-1]):
                    ...
                    lfid = f.id
                if len(pts) >= 2:
                    cv2.polylines(img, np.array([pts], dtype=np.int32), False, myjet[len(pts)] * 255, thickness=1,
                                  lineType=16)
            else:
